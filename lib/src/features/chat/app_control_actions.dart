  ...
  Future<Map<String, Object?>> _controlApp(
    ...
    if (operation == 'createConversation') {
      ...
      final value = Conversation.empty()
        ...
        ..storedTitle = title;
      await _store.writer.save(value, makeActive: false);
    ...
    final rows = await _store.database.query(
      ...
      columns: ['id'],
      where:
          "id = ? AND ((kind = 'direct' AND default_sender_id = ?) OR (kind = 'group' AND id IN (SELECT conversation_id FROM conversation_members WHERE sender_id = ? AND left_at IS NULL)))",
      whereArgs: [id, senderId, senderId],
      limit: 1,
    );
