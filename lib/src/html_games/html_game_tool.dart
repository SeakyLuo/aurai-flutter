...
class HtmlGameTool implements AgentTool, RuntimeCapabilityAgentTool {
  ...
  ToolDefinition get definition => ToolDefinition(
    ...
    description: switch (name) {
      'createHtmlGame' =>
        '在当前群聊创建一条 HTML 小游戏消息，仅在用户要求时使用。加载时只初始化并展示，不自动开始游戏、倒计时或计分；由页面内明确的用户操作（例如首次方向操作）开始，不要求额外的通用激活按钮。恢复或重新渲染不得自动开新局，普通装饰动画和时钟可正常运行。提交 HTML/CSS/JS，可用 Canvas；支持 HTTPS 远程资源和网络请求、WSS 连接，遵循浏览器 CORS 规则；远程脚本与页面共享桥接权限，只使用可信来源。禁止 iframe 和手机 API。网页使用 window.AuraiGame.snapshot 读取 {state,version,participants,turnSenderId,status}；AuraiGame.subscribe(snapshot=>render(snapshot)) 接收初始和后续状态；await AuraiGame.commit({eventId,expectedVersion,state,action,turnSenderId,status,notifySenderIds}) 提交完整下一状态，收到 applied:true 才认为成功，冲突时按返回最新状态重新决策。eventId 使用 crypto.randomUUID()，重试同一操作复用编号。状态至多64KB，HTML至多4MB。动画本地执行，不逐帧提交；只在关键回合通知指定 AI。可见时在消息内运行网页，离屏/后台释放，不能依赖 JS 内存或卸载时保存。通过 width/height 指定卡片期望尺寸，width 为 null 自适应消息宽度；实际尺寸会按可用屏幕缩小，内容必须紧凑并响应式适配消息宽度，高度首次按内容测量后固定，后续布局变化须由网页调用 AuraiHTML.requestResize() 主动申请重新测量，不要逐帧调用；使用 --aurai-text、--aurai-muted、--aurai-field、--aurai-border、--aurai-accent 适配主题。参与者包含用户和创建者，使用当前群成员ID，不能让用户填写ID。',
      'readHtmlGame' =>
    ...
    inputSchema: {
      ...
      'properties': {
        ...
        if (name == 'createHtmlGame') ...{
          ...
          'html': {'type': 'string', 'maxLength': HtmlAppStore.maxHtmlBytes},
          'width': {
            'type': ['integer', 'null'],
            'minimum': 180,
            'maximum': 600,
