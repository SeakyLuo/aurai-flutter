...
const htmlAppGuide =
    ...
    'A generated local image can become the miniapp icon: use the exact imagePath returned by generateImage with setHtmlAppIcon. It imports the image into durable miniapp storage without cropping or publishing a new code version. '
    'Library publication is separate from message creation: listHtmlAppPublications finds publishable apps by title; readHtmlAppPublication reads the release revision; publishHtmlApp publishes a first version, updateHtmlAppPublication publishes a new snapshot, and withdrawHtmlApp withdraws it. Only publish or withdraw when the user asks. These operations act on behalf of the user and never publish saved data or upload to the internet. '
    ...
    'Direct AI: await AuraiHTML.ai.getStatus() reports availability without credentials. await AuraiHTML.ai.complete({messages:[{role:"user",content:"Hello"}],responseFormat:"text",maxOutputTokens:4096},{signal,onText}) uses the app default text model and reasoning settings; resolves {text,model} or {text,json,model} for responseFormat:"json". onText receives accumulated text; signal is an AbortSignal. This is tool-free inference, not an agent task: no chat history, memory, file writes or automatic callback. Validate results and save explicitly through writeData. Requests are cancelled when the page closes/hides and expire after 120 seconds; handle errors with a toast. Do not resubmit in an update listener or fall back to a different model/account. '
